...
public class RotHandler
{
	public static ItemStack doRot(World world, ItemStack item)
	{
		...
		} else
		{
			...
			if(UBD == 0)
			{
				...
				UBD = (world.getTotalWorldTime()/EM_Settings.foodRound) * EM_Settings.foodRound;
				UBD = UBD <= 0L? 1L : UBD;
	...
	public static void rotInvo(World world, IInventory inventory)
	{
		...
		{
			EnviroRot.logger.log(Level.ERROR, "An error occured while attempting to rot inventory:", e);
			return;
		}
	}
}
