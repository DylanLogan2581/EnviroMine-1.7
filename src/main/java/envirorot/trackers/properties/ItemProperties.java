package envirorot.trackers.properties;
...
import envirorot.utils.EnviroUtils;
...
public class ItemProperties implements SerialisableProperty, PropertyBase
{
	...
	public void LoadProperty(Configuration config, String category)
	{
		...
		if(meta < 0)
		{
			...
			if(EM_Settings.itemProperties.containsKey("" + name) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EnviroRot.logger.log(Level.ERROR, "CONFIG DUPLICATE: Items - "+ name.toUpperCase() +" was already added from "+ EM_Settings.itemProperties.get(name).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
			
			EM_Settings.itemProperties.put("" + name, entry);
		} else
		{
			// If item already exist and current file hasn't completely been loaded do this
			if(EM_Settings.itemProperties.containsKey("" + name + "," + meta) && !EM_ConfigHandler.loadedConfigs.contains(filename)) EnviroRot.logger.log(Level.ERROR, "CONFIG DUPLICATE: Items - "+ name.toUpperCase() +" - Meta:"+ meta +" was already added from "+ EM_Settings.itemProperties.get(name).loadedFrom.toUpperCase() +" and will be overriden by "+ filename.toUpperCase());
	...
	public void GenDefaults()
	{
		...
		while(iterator.hasNext())
		{
			...
			if(regName.length <= 0)
			{
				EnviroRot.logger.log(Level.ERROR, "Failed to get correctly formatted object name for " + item.getUnlocalizedName());
				continue;
			}
			...
			if(!itemFile.exists())
			{
				...
				{
					EnviroRot.logger.log(Level.ERROR, "Failed to create file for " + item.getUnlocalizedName(), e);
					continue;
				}
			}
	...
	public void generateEmpty(Configuration config, Object obj)
	{
		...
		{
			EnviroRot.logger.log(Level.ERROR, "Tried to register config with non item object!", new Exception());
			return;
		}
		...
		if(regName.length <= 0)
		{
			EnviroRot.logger.log(Level.ERROR, "Failed to get correctly formatted object name for " + item.getUnlocalizedName() +"_"+ regName[1]);
			return;
		}
