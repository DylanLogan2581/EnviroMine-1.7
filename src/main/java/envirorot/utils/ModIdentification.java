...
public class ModIdentification
{
	...
	public static String idFromObject(Object obj)
	{
		...
		try
		{
			...
			if(clazz == null)
			{
				...
				EnviroRot.logger.log(Level.ERROR, "ModID lookup failed for: NULL");
				return "unknown";
			}
			...
			//EnviroRot.logger.log(Level.INFO, "ModID lookup Success for: " + (obj instanceof Class? ((Class)obj).getName() : obj.getClass().getName()) + " {" + fullPath + "}");
		...
		} catch(Exception e)
		{
			...
			//EnviroRot.logger.log(Level.INFO, "ModID lookup failed for: " + (obj instanceof Class? ((Class)obj).getName() : obj.getClass().getName()) + " {" + fullPath + "}", e);
			...
			else EnviroRot.logger.log(Level.INFO, "ModID lookup failed for: " + (obj instanceof Class? ((Class)obj).getName() : obj.getClass().getName()) + " {" + fullPath + "}");
		...
		{
			EnviroRot.logger.log(Level.WARN, "Unable to find matching ModID for " + clazz.getSimpleName());
		} 
		return modName;
	}
	...
	static
	{
		...
		for(ModContainer mod : Loader.instance().getModList())
		{
			...
			EnviroRot.logger.log(Level.INFO, "Mapped mod: {" + mod.getSource().getAbsolutePath() + "," + mod.getName() + "," + mod.getModId() + "}");
		}
	}
}
