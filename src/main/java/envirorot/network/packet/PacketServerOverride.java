...
import envirorot.core.EM_Settings.ShouldOverride;
import envirorot.core.EnviroRot;
import envirorot.trackers.properties.helpers.SerialisableProperty;
...
public class PacketServerOverride implements IMessage
{
	...
	public static class Handler implements IMessageHandler<PacketServerOverride, IMessage>
	{
		...
		public IMessage onMessage(PacketServerOverride message, MessageContext ctx)
		{
			...
			{
				EnviroRot.logger.log(Level.WARN, "AutoOverride attempted to run serverside! This should not happen!");
				return null;
			}
			...
			for(Field f : fields)
			{
				try
				{
					...
					{
						EnviroRot.logger.log(Level.WARN, "Field " + f.getName() + " is protected and cannot be synced!");
						continue;
					} else if(!Modifier.isStatic(f.getModifiers()))
					{
						EnviroRot.logger.log(Level.WARN, "Cannot sync non-static field " + f.getName() + "!");
						continue;
					}
					...
					if(f.getType() == HashMap.class)
					{
						if(clazzes.length < 2)
						{
							EnviroRot.logger.log(Level.ERROR, "Annotation for field " + f.getName() + " (" + f.getType().getName() + ") is missing class types!");
							continue;
						}
						...
						for(int i = 0; i < nbtList.tagCount(); i++)
						{
							...
							{
								EnviroRot.logger.log(Level.WARN, "Position " + i + " in HashMap " + f.getName() + " returned a null entry! Skipping...");
								continue;
							}
					...
					} else if(f.getType() == ArrayList.class)
					{
						if(clazzes.length < 1)
						{
							EnviroRot.logger.log(Level.ERROR, "Annotation for field " + f.getName() + " is missing class types!");
							continue;
						}
						...
						for(int i = 0; i < nbtList.tagCount(); i++)
						{
							...
							if(valObj == null)
							{
								EnviroRot.logger.log(Level.WARN, "Position " + i + " in ArrayList " + f.getName() + " returned a null entry! Skipping...");
								continue;
							}
					...
					} else
					{
						...
						if(value == null)
						{
							EnviroRot.logger.log(Level.WARN, "Field " + f.getName() + " returned a null value! Skipping...");
							continue;
						}
				...
				{
					EnviroRot.logger.log(Level.ERROR, "An error occured while syncing setting " + f.getName(), e);
				}
			}
		...
		public Object getNBTValue(NBTTagCompound tag, String key, Class<?> clazz)
		{
			...
			} else if(clazz.isAssignableFrom(SerialisableProperty.class))
			{
				...
				{
					EnviroRot.logger.log(Level.ERROR, "An error occured while trying to instantiate " + clazz.getSimpleName(), e);
					return null;
				}
			} else
			{
